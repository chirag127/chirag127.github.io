        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not choices or not isinstance(choices, list) or len(choices) == 0:
                    return CompletionResult(success=False, error=f"OpenRouter Error: Invalid 'choices' in response: {data}")

                message = choices[0].get("message")
                if not message:
                    return CompletionResult(success=False, error=f"OpenRouter Error: Missing 'message' in first choice: {data}")

                content = message.get("content")
                if content is None:
                    return CompletionResult(success=False, error=f"OpenRouter Error: content is null in response: {data}")
